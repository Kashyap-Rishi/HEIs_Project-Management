...
const ProjectPageVerified = ({ params: { projectCode, username, courseCode } }: Params) => {
    ...
    <ProjectDataProvider projectCode={projectCode}>
      <ProjectDataContext.Consumer>
        {(dataContextValue) => {
          ...
          const verifiedProjects = projectTopic.projects.filter((projects:any) => projects.projectStatus === 'verified');
          ...
          const startIndex = verifiedProjects.length > 0 ? (currentPage - 1) * itemsPerPage : 0;
const endIndex = Math.min(startIndex + itemsPerPage, verifiedProjects.length);
          const paginatedProjects = verifiedProjects.slice(startIndex, endIndex);

          return (
            <LayoutWrapper username={username}>
              <Paper elevation={3} sx={{ padding: 3, marginBottom: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <Typography variant="h4" gutterBottom>Accepted Submissions</Typography>
  ...
  {paginatedProjects.map((project: any, index: any) => (
    <Grid item xs={12} key={index} > {/* Adjust grid sizing as needed for responsiveness */}
      <ProjCardSI
        ...
        statusBackgroundColor="rgb(121, 240, 121)" // Ensure you have a logic to determine this color based on status
      />
    </Grid>
  ))}
</Grid>
  ...
  {verifiedProjects.length > 0 && (
    <>
      ...
      <Typography variant="body1">
        Showing {startIndex + 1}-{endIndex} of {verifiedProjects.length}
      </Typography>
      <Pagination
        count={Math.ceil(verifiedProjects.length / itemsPerPage)}
        page={currentPage}
  ...
  {verifiedProjects.length === 0 && (
    <Typography variant="body1" sx={{ mx: 'auto' }}>No accepted projects to display</Typography>
  )}
</Box>
...
export default ProjectPageVerified;

...
const ProjectPageRejected = ({ params: { projectCode, username, courseCode } }: Params) => {
    ...
    <ProjectDataProvider projectCode={projectCode}>
      <ProjectDataContext.Consumer>
        {(dataContextValue) => {
          ...
          const rejectedProjects = projectTopic.projects.filter((projects:any) => projects.projectStatus === 'rejected');
          ...
          const startIndex = rejectedProjects.length > 0 ? (currentPage - 1) * itemsPerPage : 0;
const endIndex = Math.min(startIndex + itemsPerPage, rejectedProjects.length);
          const paginatedProjects = rejectedProjects.slice(startIndex, endIndex);

          return (
            <LayoutWrapper username={username}>
              <Paper elevation={3} sx={{ padding: 3, marginBottom: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <Typography variant="h4" gutterBottom>Rejected Submissions</Typography>
  ...
  {paginatedProjects.map((project: any, index: any) => (
    <Grid item xs={12} key={index} > {/* Adjust grid sizing as needed for responsiveness */}
      <ProjCardSI
        ...
        statusBackgroundColor="rgb(247, 147, 139)" // Ensure you have a logic to determine this color based on status
      />
    </Grid>
  ))}
</Grid>
  ...
  {rejectedProjects.length > 0 && (
    <>
      ...
      <Typography variant="body1">
        Showing {startIndex + 1}-{endIndex} of {rejectedProjects.length}
      </Typography>
      <Pagination
        count={Math.ceil(rejectedProjects.length / itemsPerPage)}
        page={currentPage}
  ...
  {rejectedProjects.length === 0 && (
    <Typography variant="body1" sx={{ mx: 'auto' }}>No rejected projects to display</Typography>
  )}
</Box>
...
export default ProjectPageRejected;
